...
import { passportNumberInputConstraints, libyanNationalIdInputConstraints, decimalInputConstraints } from '@/lib/input-constraints';
...
import { index as educationMonitorStudentsIndex } from '@/routes/education-monitor/students';
import { create as createEducationMonitorTransfer, store as storeEducationMonitorTransfer } from '@/routes/education-monitor/students/transfers';
import { index as schoolStudentsIndex } from '@/routes/school/students';
import { create as createSchoolTransfer, store as storeSchoolTransfer } from '@/routes/school/students/transfers';

type AddTransferredStudentContext = 'school' | 'education-monitor';

type AddTransferredStudentFilter = {
    name?: string;
...
const config: Record<AddTransferredStudentContext, {
    ...
    school: {
        ...
        confirmDestination: 'المدرسة',
    },
};

function resolveCreateForm(context: AddTransferredStudentContext) {
    switch (context) {
        case 'education-monitor':
            return createEducationMonitorTransfer.form();
        case 'school':
            return createSchoolTransfer.form();
    }
}

function resolveCreateUrl(context: AddTransferredStudentContext): string {
    switch (context) {
        case 'education-monitor':
            return createEducationMonitorTransfer.url();
        case 'school':
            return createSchoolTransfer.url();
    }
}

function resolveStoreUrl(context: AddTransferredStudentContext): string {
    switch (context) {
        case 'education-monitor':
            return storeEducationMonitorTransfer.url();
        case 'school':
            return storeSchoolTransfer.url();
    }
}

function resolveStudentsIndexUrl(context: AddTransferredStudentContext): string {
    switch (context) {
        case 'education-monitor':
            return educationMonitorStudentsIndex.url();
        case 'school':
            return schoolStudentsIndex.url();
    }
}

type AddTransferredStudentProps = {
    students: Student[];
    filter: AddTransferredStudentFilter;
    context: AddTransferredStudentContext;
};

export default function AddTransferredStudent({ students, filter, context }: AddTransferredStudentProps) {
    const { alertDescription, confirmDestination } = config[context];
    ...
    function confirmTransfer(): void {
        form.post(resolveStoreUrl(context), {
            preserveScroll: true,
                    ...
                    <Card className={students.length > 0 ? 'gap-0' : ''}>
                        ...
                        <CardFooter className="justify-end gap-x-4 border-t">
                            <Button
                                ...
                                asChild
                            >
                                <Link href={resolveStudentsIndexUrl(context)}>
                                    <ReplyIcon />
                                ...
                                <AlertDialogContent>
                                    <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription>
                                            ...
                                            <span>طالب/طالبة إلى {confirmDestination}، هل أنت متأكد من هذا الإجراء ؟</span>
                                        </AlertDialogDescription>
                                    </AlertDialogHeader>
