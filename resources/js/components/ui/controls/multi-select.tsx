...
const multiSelectChipClassName = cn(
    "rounded-none border border-input bg-background px-2 py-0.5 text-sm font-normal normal-case tracking-normal text-foreground",
);

const multiSelectCheckboxClassName = cn(
    "flex size-4.5 shrink-0 items-center justify-center rounded-none border border-input bg-background",
    "[&_svg]:size-3.5",
);
...
/**
 * Shared optional fields for multi-select options.
 */
type MultiSelectOptionFields = {
...
/**
 * Accepted option shapes. The component normalizes these to { value, label } internally.
 */
type MultiSelectOptionWithValueLabel = MultiSelectOptionFields & {
    value: string | number;
    label: string;
};

type MultiSelectOptionWithIdName = MultiSelectOptionFields & {
    id: string | number;
    name: string;
};

type MultiSelectOptionInput = MultiSelectOptionWithValueLabel | MultiSelectOptionWithIdName;

/**
 * Normalized option used internally after resolving flexible input shapes.
 */
...
function normalizeOption(option: MultiSelectOptionInput): MultiSelectOption {
    if ("value" in option) {
        return {
...
function normalizeOptions(
    options: MultiSelectOptionInput[] | MultiSelectGroup[]
): MultiSelectOption[] | NormalizedMultiSelectGroup[] {
    if (options.length === 0) {
        return [];
    }

    if ("heading" in options[0]) {
        return (options as MultiSelectGroup[]).map(function (group) {
            return {
    ...
    return (options as MultiSelectOptionInput[]).map(normalizeOption);
}
...
interface NormalizedMultiSelectGroup {
    heading: string;
...
interface MultiSelectProps
     ...
     * Each option may use either `{ value, label }` or `{ id, name }`.
     */
    options: MultiSelectOptionInput[] | MultiSelectGroup[];
...
export const MultiSelect = React.forwardRef<MultiSelectRef, MultiSelectProps>(
    (
        ...
        const isGroupedOptions = React.useCallback(
            (
                opts: MultiSelectOption[] | NormalizedMultiSelectGroup[]
            ): opts is NormalizedMultiSelectGroup[] => {
        ...
        }, [normalizedOptions, deduplicateOptions, isGroupedOptions]);
        ...
        }, [normalizedOptions, searchValue, searchable, isGroupedOptions]);
            ...
            <>
                ...
                <Popover
                        ...
                        <Button
                                ...
                                {selectedValues.length > 0 ? (
                                    <div
                                        className={cn(
                                            ...
                                            singleLine
                                                ? "no-scrollbar items-center overflow-x-auto overflow-y-hidden"
                                                : "max-h-full flex-wrap items-center overflow-hidden",
                                        ...
                                        {selectedValues.length > responsiveSettings.maxCount && (
                                            <Badge
                                                className={cn(
                                                    multiSelectChipClassName,
                                                    "border-input bg-transparent font-mono text-muted-foreground",
                                                    getBadgeAnimationClass(),
                                                    multiSelectVariants({ variant }),
                                                    responsiveSettings.compactMode &&
                                                    "px-1.5 py-0.5 text-xs",
                            ...
                            <div
                                ...
                                <div
                                    ...
                                    className={cn(
                                        "flex size-7 shrink-0 cursor-pointer items-center justify-center rounded-none text-muted-foreground transition-colors hover:text-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary/50",
                                        selectedValues.length === 0 &&
                                ...
                                <Separator
                                    ...
                                    className={cn(
                                        "mx-1 h-[1.875rem] shrink-0",
                                        selectedValues.length === 0 && "invisible",
                                    )}
                                />
                    ...
                    <PopoverContent
                        ...
                        className={cn(
                            "w-auto rounded-none p-0 shadow-md ring-1 ring-foreground/10",
                        ...
                        <Command>
                            ...
                            <CommandList
                                className={cn(
                                    "no-scrollbar max-h-[40vh] overflow-y-auto",
                                    screenSize === "mobile" && "max-h-[50vh]",
                                    "overscroll-behavior-y-contain",
                                )}>
                                <CommandEmpty className="py-3 text-center text-sm">
                                    {emptyIndicator || "لا توجد نتائج"}
                                </CommandEmpty>{" "}
                                {!hideSelectAll && !searchValue && (
                                    <CommandGroup className="border-b border-border/50">
                                        <CommandItem
                                            key="all"
                                            onSelect={toggleAll}
                                            role="option"
                                            ...
                                            <span>
                                                ( اختر الكل {getAllOptions().length > 20 ? ` - ${getAllOptions().length} خيار` : ""} )
                                            </span>
                                        </CommandItem>
                                    </CommandGroup>
                                )}
                                {isGroupedOptions(filteredOptions) ? (
                                    ...
                                    <CommandGroup className="[&>div]:space-y-0.5">
                                        ...
                                        {selectedValues.length > 0 && (
                                            <>
                                                <CommandItem
                                                    ...
                                                    className="flex-1 cursor-pointer justify-center rounded-none [&>svg:last-child]:hidden">
                                                    مسح الكل
                                                </CommandItem>
                                                <Separator
                                                    orientation="vertical"
                                                    className="mx-0.5 h-9"
                                                />
                                            </>
                                        )}
                                        <CommandItem
                                            onSelect={() => setIsPopoverOpen(false)}
                                            className="max-w-full flex-1 cursor-pointer justify-center rounded-none [&>svg:last-child]:hidden">
                                            إغلاق
                                        </CommandItem>
                                    </div>
                                </CommandGroup>
...
export type { MultiSelectOption, MultiSelectOptionInput, MultiSelectGroup, MultiSelectProps };
