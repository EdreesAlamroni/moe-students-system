...
beforeEach(function () {
    PolicyRegistrar::register(Request::create('/education-monitor/reports/student-count-by-grade-level', 'GET'));
...
test('guests are redirected from the student count by grade level report page', function () {
    $this->get(route('education-monitor.reports.student-count-by-grade-level.index'))
...
test('users without student count by grade level report permissions cannot view the report', function () {
    ...
    $this->actingAs($user, 'education_monitor')
        ->get(route('education-monitor.reports.student-count-by-grade-level.index'))
        ->assertForbidden();
});
...
test('authenticated users can visit the student count by grade level report page', function () {
    ...
    $this->actingAs($user, 'education_monitor')
        ...
        ->assertInertia(fn (Assert $page) => $page
            ->component('education-monitor/reports/student-count-by-grade-level')
...
test('guests are redirected from the student count by grade level report print page', function () {
    $this->get(route('education-monitor.reports.student-count-by-grade-level.print'))
        ->assertRedirect(route('education-monitor.login'));
});
...
test('users without print permission cannot print the student count by grade level report', function () {
    ...
    Permission::findOrCreate('report:student-count-by-grade-level:view', UserScope::EDUCATION_MONITOR->value);
    $user->givePermissionTo('report:student-count-by-grade-level:view');

    $this->actingAs($user, 'education_monitor')
        ->get(route('education-monitor.reports.student-count-by-grade-level.print'))
        ->assertForbidden();
});
...
test('authenticated users can print the student count by grade level report', function () {
    ...
    $this->actingAs($user, 'education_monitor')
        ->get(route('education-monitor.reports.student-count-by-grade-level.print'))
        ->assertOk()
        ->assertViewIs('print.education-monitor.reports.student-count-by-grade-level')
